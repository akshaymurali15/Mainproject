...
<body>
  ...
  <form method="POST" name="myform" onsubmit="return validate();" enctype="multipart/form-data" action="#">
      ...
      function getAge() {
...

        {
...

          {
            ...
            alert("Age " + age + " is restrict Allowed only for more than 1 years old");
    ...
    <script>
      function Validateaaa() {
        var val = document.getElementById('na').value;

        if (!val.match(/^[0-9]{12}/)) {
          document.getElementById('dha').innerHTML = "Only Numbers are allowed and must contain 12 number";
    ...
    <div class="option-box">
      <div class="selectBox">
        ...
        <select name="role">
          ...
          <option value="4">Deliveryboy</option>
        </select>
      </div>
      <div class="sub">
...
if (isset($_POST['submit'])) {
  ...
  if (mysqli_num_rows($res) > 0) {
    ...
    $log = mysqli_query($conn, "INSERT INTO `login`(`username`, `password`, `Role`) VALUES ('$usrnm','$pssrd','$Role')");
    ...
    if ($Role == "0") {
      $sql1 = mysqli_query($conn, "INSERT INTO `user`(  `Name`, `Mobile`, `Email`,`Login_id`, `Address`, `D.O.B`, `Aadhaar_no`, `Aadhaar_Upload`,`Status`) VALUES ('$fname','$nmbr','$eml','$roleID','$addr','$dobi','$adno','$file','1')");
    } else if ($Role == "2") {
