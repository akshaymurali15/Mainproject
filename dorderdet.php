...
include 'db.php';
$idd=$_GET['id'];
$resltt= mysqli_query($conn,"SELECT `total_price`,`Order_date`,`Status` FROM `user-order` WHERE User_Order_id='$idd'");
$ress=mysqli_query($conn,"SELECT `plo_id`, `User_Order_id`, `book_id`, `book_name`,`quantity` FROM `place_order`WHERE User_Order_id='$idd'");
$synn=mysqli_query($conn,"SELECT * FROM `user-order` JOIN `deliveryboy` ON deliveryboy.Login_id=`user-order`.deliboy_id WHERE `user-order`.Status='Shipped' and User_Order_id='$idd'");
